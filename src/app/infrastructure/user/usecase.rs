...
impl UserUseCase for UserUseCaseImpl {
    ...
    ) -> Result<user::usecase::responses::UserUsecase, AppError> {
        let user = self
            .user_repository
        ...
        match user {
            Some(user) => {
                ...
                if hasher::verify(&request.naive_password, &user.password)? {
                    ...
                    Ok(user::usecase::responses::UserUsecase::from((
                        user, token,
                    )))
                } else {
                    error!(
    ...
    async fn signup(
        ...
        let user = self
            .user_repository
            .create_user(user::repository::requests::CreateUserRepoRequest {
    ...
    async fn update_user(
        &self,
        (user_id, request): (Uuid, user::usecase::requests::UpdateUserUsecaseRequest),
    ) -> Result<user::usecase::responses::UserUsecase, AppError> {
        info!("Update user {:?}", user_id);
        let user = self
            .user_repository
            .update_user(user::repository::requests::UpdateUserRepoRequest::try_from((
                user_id, request,
            ))?)
            .await?;
